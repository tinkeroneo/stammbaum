...
let pinch = null;
let collapsed = new Set(readJsonStorage(localStorage, storeKey + '-collapsed', []));
...
async function loadPersistedDataIfAvailable() {
  ...
  if (hasPersistedTreeData) {
    schedulePersistedJsonWrite(serializeTree(data, 2));
    return true;
  }
  return false;
}
...
function clampView() {
  ...
  view = clampViewport(view, bounds, main.clientWidth, main.clientHeight);
}
function applyView() {
  clampView();
...
function render() {
  ...
  reconcileKeyedChildren(nodes, desiredCards);
...
function renderGenerationBands(visiblePeople) {
  ...
  const rows = groupRowsByTolerance(ys);
